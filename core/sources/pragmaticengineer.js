...
const {P_LIMIT, SOURCE_PRAGMENGINEER} = require('../constants');
...
const getAllPosts = async () => {
  try {
    const response = await fetch(SOURCE_PRAGMENGINEER);
    const body = await response.text();

    const $ = cheerio.load(body);

    const previous = $('.previous-articles li a')
      .map((i, element) => {
        return url.resolve(SOURCE_PRAGMENGINEER, $(element).attr('href'));
      })
      .get();

    const current = $('#content .post-title a')
      .map((i, element) => {
        return url.resolve(SOURCE_PRAGMENGINEER, $(element).attr('href'));
      })
      .get();
...
const parse = async (post, issue) => {
  try {
    ...
    const body = await response.text();
    const $ = cheerio.load(body);
    const {domain} = parseDomain(SOURCE_PRAGMENGINEER);
